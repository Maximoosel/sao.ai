    ...
    <div className="min-h-screen bg-[hsl(235,24%,8%)] flex items-center justify-center px-6">
      ...
      <motion.div className="w-full max-w-sm relative z-10" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <div className="rounded-2xl border border-white/[0.06] bg-white/[0.03] backdrop-blur-2xl p-8">
          ...
          <p className="text-sm text-white/30 mb-6">Enter your email to receive a reset link.</p>
          {sent ? (
            <p className="text-sm text-primary">Check your email for the reset link.</p>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-4">
              <Input type="email" placeholder="Email" value={email} onChange={(e) => setEmail(e.target.value)} required className="bg-white/[0.04] border-white/[0.08] text-foreground placeholder:text-white/20 rounded-xl h-11" />
          ...
          <button onClick={() => navigate('/auth')} className="text-sm text-primary font-medium hover:underline mt-4 block text-center w-full">Back to sign in</button>
        </div>
      </motion.div>
    </div>
  );
